<!-- /src/routes/scales/inner/[slug]/+page.svelte -->
<script lang="ts">
  ...
  const translations = {
    en: {
      ...
      backToInner: "Back to Inner Commons",
      innerCommons: "Inner Commons"
    },
    sv: {
      fallbackNotice: "OBS: Svensk översättning inte tillgänglig ännu. Visar engelsk version.",
      backToInner: "Tillbaka till Inre Allmänningar",
      innerCommons: "Inre Allmänningar"
    }
  };
  ...
  <div class="max-w-4xl mx-auto px-4 py-16">
    ...
    <nav class="mb-8 text-sm">
      <ol class="flex items-center gap-2 text-earth-600">
        ...
        <li>
          <a href="{base}/scales/inner" class="hover:text-moss-600 transition-colors">{t.innerCommons}</a>
        </li>
        <li class="text-earth-400">/</li>
      ...
      <div class="flex items-center gap-4 text-sm text-earth-500">
        ...
        {#if data.metadata.date}
          ...
          <time datetime={data.metadata.date}>
            {new Date(data.metadata.date).toLocaleDateString($language === 'sv' ? 'sv-SE' : 'en-US', {
              year: 'numeric',
              month: 'long',
              day: 'numeric'
            })}
